...
int main( int argc, char *argv[] )
{
  ...
  // Time a run ...
  clock_t  begin = clock();
  long int iters = insertion( A, n );
  clock_t  end   = clock();

  double elapsed = (double) ( end - begin ) / CLOCKS_PER_SEC;
  int numErrs = checkArray( A, n );

  printf( "insertion( ascending, %'13d ) => %'5d err%s, %'26ld while iter%s, %'11.3fs\n",
    n,
    numErrs, numErrs == 1 ? "" : "s",
    iters, iters == 1 ? "" : "s",
    elapsed );
  ...
  printf( "insertion( shuffle,   %'13d ) => %'5d err%s, %'26ld while iter%s, %'11.3fs\n",
    n,
    numErrs, numErrs == 1 ? "" : "s",
    iters, iters == 1 ? "" : "s",
    elapsed );
